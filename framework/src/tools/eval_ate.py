...
def evaluate_ate(first_list, second_list, plot="", _args=""):
    ...
    noaligned_error = np.asarray((second_xyz-first_xyz)).sum(axis=0) 
    noaligned_ate   = np.sqrt( numpy.dot(noaligned_error,noaligned_error)/ len(noaligned_error) )
    ...
    if args.plot:
        ...
        # plot_traj(ax, first_stamps, first_xyz_full.transpose().A, '+', "black", "ground truth")
        plot_traj(ax, second_stamps, second_xyz_full_aligned.transpose().A, '-x', "blue", "estimated")

        label = "difference"
        for (a, b), (x1, y1, z1), (x2, y2, z2) in zip(matches, first_xyz.transpose().A, second_xyz_aligned.transpose().A):
            # ax.plot([x1,x2],[y1,y2],'-',color="red",label=label)
            label = ""
        ax.set_title(f'Aligned ATE RMSE:{ATE}, frame #:{len(trans_error)} ')
        ax.legend() 
        ax.set_xlabel('x [m]')
        ax.set_ylabel('y [m]')
        ...
        ax.set_title(f'len:{len(trans_error)} ATE RMSE:{ATE} ')
        plot_traj(ax, first_stamps, first_xyz_full.transpose().A, '-+', "black", "ground truth")
        plot_traj(ax, second_stamps, second_xyz_full.transpose().A, '-x', "green", "estimated")

        ax.set_title(f'NotAligned ATE RMSE:{noaligned_ate}, frame #:{len(trans_error)} ')
        ax.legend()
        ax.set_xlabel('x [m]')
        ax.set_ylabel('y [m]')
        ax.set_aspect('equal', 'box')
