...
def sigma2weights(z_vals, sigma, valid):
    ...
    if valid is not None:    
        assert valid.shape == sigma.shape
        
        sigma2 = torch.zeros_like(sigma)
        sigma2[valid] = sigma[valid]
        sigma  = sigma2 

    free_energy = dists * sigma

    # probability of it is not empty here
    alpha = 1 - torch.exp(-free_energy) 

    # shift one step
    #_b = torch.zeros((N, 1),device=device) 
    _b = torch.zeros(alpha.shape[:-1],device=device,dtype=torch.float32)
    _b = _b.unsqueeze(-1)
    shifted_free_energy = torch.cat([_b, free_energy[..., :-1]], dim=-1)  
...
def volume_rendering(sigma, rgb, z_vals, valid, device, weight_only=False): 

    # N, M
    weights, alpha, transmit= sigma2weights(z_vals, sigma, valid) 
...
def z_inverse_samples(ray_o, ray_d, ray_ti, ray_near, ray_far, upsample_num, stepsize,   sce_data, decoder, motion_net, deform, use_unifrom_ini,   use_rand_ini, device, max_pre_samples=4096, min_pre_samples=64,
    ...
    for _ in range(iterations):
        ...
        _sigma = out['sigma']   
        _sigma = rearrange( _sigma, '(n m) 1 -> n m',n=N,m=M)
        
        #_invs= out['p_invs']
        #_invs = rearrange( _invs, '(n m) 1 -> n m',n=N,m=M)

        # n,m-1
        weights,_,_ = sigma2weights(i_z, _sigma, valid=None)
...
def rays_search_z_single_obj(ray_o, ray_d, ray_ti, ray_tnear, ray_tfar, sample_mode, snum, sce_data, decoder, motion_net, 
    ...
    sigma= rearrange(sigma,'(n m) 1 -> n m'  , n=N ,m=snum)
...
def render_rays_w_objs( 
    ...
    # k f n m
    _ort = volume_rendering(  sigma=obj_all_sigma,
                              rgb=obj_all_rgb, 
                              z_vals=obj_all_z,
                              valid=obj_all_valid,
                              device=device )
    ...
    #----------------------------------------------
    rt = volume_rendering(st_sigma, st_rgb, st_z,  st_valid, device)
