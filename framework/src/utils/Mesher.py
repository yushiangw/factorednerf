...
class Mesher(object):
    ...
    def __init__(self, cfg, args, slam, points_batch_size=50000, ray_batch_size=50000):
        """
        ...
        self.frame_reader = get_dataset(cfg, args, scale=1.0, device='cpu')
    ...
    def point_masks(self, input_points, keyframe_dict, estimate_c2w_list, idx, device, get_mask_use_all_frames=False):
        """
            ...
            if get_mask_use_all_frames:
                ...
                for keyframe in keyframe_dict:
                    ...
                    if self.depth_test:
                        ...
                        cur_mask_seen[cur_mask_seen.clone()] &= proj_depth_seen < max_depth
    ...
    def get_tsdf(self, keyframe_dict, scale=1):
        """
    ...
    def eval_points_v2(self, p, decoders, c=None, stage='color', device='cuda:0'):
        """
        ...
        for pi in p_split:
            ...
            ret = decoders(pi, scene_model=c, stage=stage)
        ...
        with torch.no_grad():
            ...
            if show_forecast:
                ...
                for i, pnts in enumerate(
                    ...
                    z_forecast.append( self.eval_points_v2(pnts, decoders, c, 'coarse', device).cpu().numpy()[:, -1])
                ...
                z_seen = []
                for i, pnts in enumerate(torch.split(seen_points, self.points_batch_size,dim=0)):
                    z_seen.append(
                        self.eval_points_v2(pnts, decoders, c, 'fine',
                                         device).cpu().numpy()[:, -1])
                ...
                mesh_bound = self.get_bound_from_frames( keyframe_dict, 1.0)
                z = []
                mask = []
                ...
                for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                    z.append(self.eval_points_v2(pnts, decoders, c, 'fine',
                                              device).cpu().numpy()[:, -1])
            ...
            if clean_mesh:
                ...
                if self.get_largest_components:
                    ...
                    for comp in components:
                        if comp.area > self.remove_small_geometry_threshold * 1.0:
