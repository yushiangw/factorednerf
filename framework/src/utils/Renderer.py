import torch
import pdb 
from src.common import get_rays, raw2outputs_nerf_color_v2, sample_pdf
...
class Renderer(object):
    def __init__(self, cfg, args, slam, points_batch_size=50000, ray_batch_size=50000):
        ...
        self.occupancy = True #cfg['occupancy']
    ...
    def eval_points_v2(self, p, decoders, c, stage='color', device='cuda:0'):
        """
    ...
    def eval_points_v3(self, p, decoders, c, stage='color', device='cuda:0'):
        """
        Evaluates the occupancy and/or color value for the points.
        ...
        for pi in p_split:
            ...
            ret =torch.cat((ret['rgb'], ret['occ']),dim=-1)
    ...
    def render_batch_ray(self, c, decoders, rays_d, rays_o, device, stage, gt_depth=None):
        ...
        pointsf = pts.reshape(-1, 3).to(torch.float32)
        
        raw = self.eval_points_v3(pointsf, decoders, c, stage, device)
        raw = raw.reshape(N_rays, N_samples+N_surface, -1)

        depth, uncertainty, color, weights = raw2outputs_nerf_color_v2(
        ...
        if N_importance > 0:
            ...
            pts = pts.reshape(-1, 3).to(torch.float32)

            raw = self.eval_points_v2(pts, decoders, c, stage, device)
