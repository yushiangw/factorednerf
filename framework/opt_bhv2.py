...
from src.common import torch_load, random_nd_index, plot_loss, save_ray_log, count_parameters
...
from src.utils import Renderer_neus_gen_v2_dw

from src.optvis_tools import vis_geo_and_save_render_mask, make_video 
...
class Helper():
    ...
    def __init__(self, cfg, args, cfg_name, no_dir=False):
        ...
        #===========================
        db_mod   = __import__(cfg['dataset']['module'], fromlist=[cfg['dataset']['class']])
        ...
        elif isinstance(self.shared_cfg.opt_frames_num,list):
            frange = self.shared_cfg.opt_frames_num 
            assert len(frange)==3
            
            self.opt_frames_indices = [i for i in range(frange[0],frange[1],frange[2])]

            num = frange[1]-frange[0]
            assert num<=len(self.frame_reader_train)

        else:
            raise Exception('error')


        print('self.opt_frames_indices')
        print(self.opt_frames_indices)
        #======================================================
        ...
        if pinit_m in ['GT', 'noise_GT_t2', '1stframe' ]: 
            raise Exception('unsupported')

        elif pinit_m in ['eye','est_pose_v1']: 
    ...
    def opt_vis_pose(self, sub_save_dir, prefix, cur_fidx, obj_kf_poses, obj_uids, obj_idx2tidx, step=None, max_len=None, post_fix=''):
        ...
        raise Exception('need to update gt')
    ...
    def render_all(self, sub_save_dir, prefix, keyframes, keyframes_indices, obj_sce_data_list, obj_bounds_world, obj_kf_poses, obj_idx2tidx  ):
        ...
        # save mp4 
        # if len(img_list)>0:
        #     save_fp = os.path.join(sub_save_dir,f'video_{prefix}.gif' ) 
        #     make_gif(img_list, save_fp, fps=3)
        #
        #     save_fp = os.path.join(sub_save_dir,f'video_{prefix}.mp4' ) 
        #     make_video(img_list, save_fp, fps=3)
    ...
    def collect_samples_cam_s(self, mask_mode, optimize_frames, rand_sample_num, sel_obj_uids):
        ...
        for frame in optimize_frames:
            gt_depth = frame['depth'].clone().detach().cpu()
            gt_color = frame['color'].clone().detach().cpu()
            gt_seg   = frame['seg'].clone().detach().cpu()
            gt_nv    = frame['normal'].clone().detach().cpu() 
            pose     = torch.eye(4) 
            
            data_dev=gt_depth.device
            ...
            for k in range(len(sel_obj_uids)):                
                ...
                if scfg.train_sample_alg=='v3_sample_mask': 
                    ...
                    rt = get_samples_v3_my( sample_pix_num, 
                                            ...
                                            obj_depth, gt_color, gt_seg, data_dev)
                    ...
                    p_mask2 = (gt_seg==uid).to(data_dev) # obj
        ...
        batch_rays_o = torch.stack(batch_rays_o_list,0).to(device)

        # ray points on the image plane (depth = 1)
        batch_rays_d = torch.stack(batch_rays_d_list,0).to(device)

        batch_gt_depth = torch.stack(batch_gt_depth_list,0).to(device)
        batch_gt_color = torch.stack(batch_gt_color_list,0).to(device)
        batch_gt_seg   = torch.stack(batch_gt_seg_list,0).to(device) 
        batch_gt_nv = torch.stack(batch_gt_nv_list,0).to(device)
        batch_im_iw = torch.stack(batch_im_iw_list,0).to(device)
        batch_im_ih = torch.stack(batch_im_ih_list,0).to(device)
    ...
    def sample_frame(self, keyframes, keyframes_indices, rand_sample_num, sel_obj_uids, obj_idx2tidx):
        ...
        for tk,val in obj_samples.items():
            obj_samples[tk] = torch.stack(val,0).to(device)
    ...
    def opt_mulobj_helper(self, opt_cfg, lswt, cam_rays, f_indices, obj_kf_poses, obj_uids, obj_bounds_world, obj_sce_data_list, network, obj_idx2t, max_pre_samples, device, step=None):
        ...
        if lswt.deform_eikonal is not None:
            ...
            for k in range(len(obj_sce_data_list)):
                ...
                rand_p_tidx= torch.randint(low=0, high=fnum, size=(random_p.shape[0],)) 
    ...
    def f2f_map(self, 
        ...
        # reset LR
        #self.reset_lr_by_idxs(optimizer, opt_cfg.ini_sce_lr, [sce_pg_idx] ) 
        #self.reset_lr_by_idxs(optimizer, opt_cfg.ini_net_lr, [net_pg_idx] )
    ...
    def opt_mapping(self, sce_models, obj_kf_poses, opt_cfg, keyframes, keyframes_indices  ):
        ...
        #     if kf['idx'] in scfg.vis_frames_indices:
        #         vis_keyframes.append(kf)
        #         vis_keyf_indices.append(keyframes_indices[i])

        #---------------------------------------

        if opt_cfg.pretrain_single_obj:

            assert not load_ck

            print('[pretrain_single_obj]')

            s_opt_cfg=SimpleNamespace(**opt_cfg.single_obj)

            for k in range(len(sce_models)):

                print('single object mapping :', k)
                k_bd = scfg.g_obj_bounds[k].unsqueeze(0).clone()

                rt = self.f2f_map(start_iters=start_iters,
                                  total_iters=s_opt_cfg.total_iters,
                                  sce_models=[sce_models[k]],
                                  obj_kf_poses=[obj_kf_poses[k]],
                                  obj_bounds=k_bd,
                                  obj_uids=[scfg.g_obj_uids[k]],
                                  obj_idx2t=[scfg.g_obj_idx2t[k]],
                                  opt_cfg=s_opt_cfg, 
        ...
        assert not opt_cfg.run_BA
        
        # if opt_cfg.run_BA:
        #     BA_opt_cfg=SimpleNamespace(**opt_cfg.BA)
        #     obj_bounds = scfg.g_obj_bounds.clone()
        #     rt = self.f2f_map(start_iters=0,
        #                       total_iters=BA_opt_cfg.total_iters,
        #                       sce_models=sce_models,
        #                       obj_kf_poses=obj_kf_poses,
        #                       obj_bounds=obj_bounds,
        #                       obj_uids=scfg.g_obj_uids,
    ...
    def run_db_init(self, mode):
        ...
        for idx in self.opt_frames_indices:
            ...
            #--------------
            #fwflow,bkflow = frame_reader.load_flow(idx)
    ...
    def run_build_poses(self, frame_reader, pose_init_mode):
        ...
        for k in range(len(scfg.g_obj_uids)):
            ...
            for i,idx in enumerate(k_fidxs):
                ...
                #------------------------------------------------
                # if scfg.gt_pose_mode == 'frame0':
                #     obj_gt_poses[k_uid].append(nv_gt_c2w_np)
                # elif scfg.gt_pose_mode == 'world':
    ...
    def run_train(self): 
        ...
        # set rigid pose
        #obj_gt_poses, 
        obj_ini_poses = self.run_build_poses(
    ...
    def run_eval(self, run_train=True, run_val=True,dw=1): 
        ...
        step = int(np.round(len(self.opt_frames_indices)/10))
        step = max(step,1)

        eval_fidx = [] 
        for x in range(0,len(self.opt_frames_indices)-2, step):
            eval_fidx.append(x) 
    ...
    def get_k1_tx(self):
        ...
        camk1_to_camk0 = np.asarray( [[0.10782797084662415, 0.013826288528525757, 0.9940734190433951, 2.0119251679252157], [0.021443814588189444, 0.9996383141344856, -0.016229717504339842, -0.04407698606656445], [-0.9939382734953953, 0.0230667435909217, 0.10749248262601474, -1.6846167359652109], [0.0, 0.0, 0.0, 1.0]])
        
        return camk1_to_camk0
    ...
    def eval_helper(self, data_mode, eval_fidxs,dw): 
        ...
        if data_mode=='train':
            ...
            k1_tx=self.get_k1_tx()

            k1_tx=torch.from_numpy(k1_tx).to(torch.float32).to(device)
            
            # all objects are required to be update
            for k in range(len(scfg.g_obj_uids)): 
                k_pose  = obj_kf_poses[k]
                k_uid   = scfg.g_obj_uids[k] 

                all_poses = k_pose.get_all_rigid_poses() 
                
                for i,idx in enumerate(self.opt_frames_indices): 
                    pose = all_poses[i]

                    pose2 = k1_tx @ pose 
                    k_pose.ini_rigid_pose(i, pose2)  
        else:
            raise Exception('error')
        ...
        pkl_save_fp = os.path.join(render_sdir, f'{data_mode}_v3.pkl')
        print('start')
    ...
    def render_helper(self, data_mode, eval_fidxs, dw):
        ...
        #----------------------------------------
        # reset poses 
        if data_mode=='train' or data_mode =='tr_debug':
            frame_reader = self.frame_reader_train
